...
Lavish is optional but useful for interactive review of HTML artifacts.

1. From the directory containing the figure project, run:

```bash
npx -y lavish-axi index.html
```

2. Ask the user to annotate elements or selected text directly in the browser.
3. Poll for feedback when running an interactive agent loop:

```bash
npx -y lavish-axi poll index.html
```

4. Revise `index.html`, `styles.css`, and local assets.
5. Repeat until layout warnings and user review feedback are resolved.
